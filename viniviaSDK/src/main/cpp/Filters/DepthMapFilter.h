#ifndef VINIVIA_SDK_DEPTH_MAP_FILTER_H
#define VINIVIA_SDK_DEPTH_MAP_FILTER_H
...
{
    namespace Filters::DepthMapFilter
    {
        // Vertex Shader
        ...
        constexpr char FRAGMENT_SHADER_SRC[] = R"SRC(
            ...
            void main() {
                ...
                float depthValue = originalColor.r;

                // Set a threshold for the depth value
                float threshold = 0.4; // We can addject this value depend the requirements

                // Compare the depth value to the threshold
                if (depthValue < threshold) {
                    gl_FragColor = vec4(1.0); // Near objects are white
                } else {
                    gl_FragColor = vec4(0.0); // Far objects are black
                }
            }
        )SRC";
    }

    class DepthMapFilter : public BaseFilter {
    public:
        /**
         * C-tor
         */
        DepthMapFilter() : BaseFilter() {}

        /**
         * D-tor
         */
        ~DepthMapFilter() override {}
...
#endif //VINIVIA_SDK_DEPTH_MAP_FILTER_H