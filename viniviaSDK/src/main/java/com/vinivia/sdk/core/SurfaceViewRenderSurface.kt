...
/**
 * Utilities for instantiating a [SurfaceView] and attaching to an [ViniviaRenderer].
 */
object SurfaceViewRenderSurface {
    private const val TAG = "SurfaceViewRenderSurface"

    /**
     * Inflates a non-blocking [SurfaceView] into the provided [ViewStub] and attaches
     * it to the provided [ViniviaRenderer].
     *
     *
    ...
    private fun inflateWith(
        viewStub: ViewStub,
        renderer: ViniviaRenderer, nonBlocking: Boolean
    ): SurfaceView {
        Log.d(
            TAG,
            "Inflating SurfaceView into view stub (non-blocking = $nonBlocking)."
        )
        if (nonBlocking) {
        ...
        surfaceView.holder.addCallback(object : SurfaceHolder.Callback2 {
            ...
            override fun surfaceDestroyed(holder: SurfaceHolder) {
                ...
                val detachFuture: ListenableFuture<Void?> = renderer.detachOutputSurface()
                if (!nonBlocking) {
                    try {
                        detachFuture.get()
